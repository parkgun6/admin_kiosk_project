...
<div class="row">
	<div class="col-12">
		<div class="card">
			<h4 class="card-title">
				<input type="hidden" name="buno" value="${board.buno }"
...
<form class='actionForm' action="/board/list" method="get">
	<input type="hidden" name="page" value="${pageDTO.page }"> <input
		...
		type="hidden" name="buno" value="${board.buno }">
...
console.log(card)
//const bunoval = table.querySelector("input[name=buno]").value
...
//console.log(table.querySelector("input[name=buno]").value)
...
function movePage(){
	//alert("aaaa");
	actionForm.setAttribute("action","/owner/community/free/read")
	actionForm.submit();
}
/* ------------------------------목록이동------------------------------ */
function moveList(){
...
/* ------------------------------수정Ajax------------------------------ */
 function sendAjax(data){
	...
	return fetch("/owner/community/free/modify",
		{method:"post",
...
dqs(".modBtn").addEventListener("click",function(e){
	...
	const bunoval = card.querySelector("input[name=buno]").value
	...
	const data = {buno:bunoval,title:titleval,content:text,writer:writerval,category:categoryval};
...
dqs(".removeBtn").addEventListener("click",function(){
	console.log($("input[name='buno']").val())
	sendRemoveAjax()
},false);
/* ---------------------------------------------------------------------------------- */
/* -----------------------------------수정완료(파일업로드)-------------------------------- */
...
document.querySelector(".modBtn").addEventListener("click", function(e){
	...
	const categoryval = card.querySelector("input[name=category]").value
	const text = quill.root.innerHTML;
	const data = {buno:bunoval,title:titleval,content:text,writer:writerval,category:categoryval,fileList: arr};
...
/* 	return fetch("/owner/free/modify",
			{method:"post",
